...
**Learning:** In TUI applications, missing empty states should be replaced with helpful guidance and call-to-actions (e.g., 'No domains managed yet. Press 'a' to add one.') to improve usability. In paginated Bubble Tea TUI views, when rendering an empty state message, adjust the blank space filler loop to account for the rendered message lines (e.g., set numRendered = 1) to prevent layout jumping and preserve exact pagination height.
**Action:** Always provide actionable empty states in TUI lists and account for their line height in fixed-size paginated layouts to avoid layout shifts.
