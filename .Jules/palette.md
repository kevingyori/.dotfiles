...
**Learning:** In TUI applications, missing empty states should be replaced with helpful guidance and call-to-actions. Furthermore, empty state messaging should be contextually differentiated to clarify whether the overall list is completely empty (e.g., 'No domains managed yet') versus when a search filter simply yields no results (e.g., 'No domains match your search').
**Action:** Always implement contextual empty states in list views to guide the user.
