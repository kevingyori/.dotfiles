...
## 2026-06-25 - [Contextual Empty States]
**Learning:** In TUI applications, empty state messaging should be contextually differentiated to clarify whether the overall list is completely empty (e.g., 'No items managed yet') versus when a search filter simply yields no results (e.g., 'No items match your search'). This helps users understand why they are seeing an empty view.
**Action:** Always provide specific empty state messages rather than generic 'nothing to show' indicators, especially when search/filtering functionality is present.
