...
## 2026-06-11 - [Contextual Empty States in TUI]
**Learning:** In TUI applications, empty states can be confusing if they don't differentiate between "no items exist" and "no items match your search". Users might think they deleted everything when they actually just have a restrictive filter applied.
**Action:** Always provide contextually aware empty state messaging that clarifies whether the list is completely empty or just filtered out, and ensure blank space filler loops account for the message lines to prevent layout jumping.
