...
## 2026-03-15 - [Contextual Empty States]
**Learning:** In TUIs, empty states without feedback can make the interface feel broken, especially during filtering. Users need to know whether the entire list is empty or if their current search yielded no results.
**Action:** Always provide contextually differentiated empty state messages (e.g., "No items match your search" vs "No items managed yet") and adjust layout padding to prevent jank when rendering empty messages.
