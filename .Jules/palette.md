...
## 2026-05-23 - [Contextual Empty States]
**Learning:** In TUI applications, a completely empty list can look like a bug or incomplete load if there's no messaging. Furthermore, an empty list due to a search filter feels different than an empty list because there are no items at all.
**Action:** Always provide contextual empty states. Differentiate between "No items managed yet" and "No items match your search" to clarify system state.
