...
## 2026-03-27 - [Empty State Clarity]
**Learning:** In TUI applications, empty state messaging should be contextually differentiated to clarify whether the overall list is completely empty (e.g., 'No items managed yet') versus when a search filter simply yields no results (e.g., 'No items match your search').
**Action:** When a paginated Bubble Tea TUI view renders an empty state message, adjust the blank space filler loop to account for the rendered message lines (e.g., set `numRendered = 1`) to prevent layout jumping and preserve exact pagination height. Replace missing empty states with helpful guidance and call-to-actions.
