...
## 2026-03-13 - [Explicit Empty States in TUI]
**Learning:** Rendering blank space for empty lists (especially when filtering) leaves users wondering if the app is broken or still loading. Providing explicit empty state messages (e.g., "No items match your search") provides necessary feedback that the app is functioning properly.
**Action:** Always implement explicit visual empty states for dynamic lists, ensuring layout height remains consistent so the UI doesn't collapse.
