...
**Learning:** In TUI applications, empty state messaging should be contextually differentiated to clarify whether the overall list is completely empty (e.g., 'No items managed yet') versus when a search filter simply yields no results (e.g., 'No items match your search'). Missing empty states should be replaced with helpful guidance and call-to-actions to improve usability.
**Action:** Always implement contextual empty states in TUIs and adjust space filler loops to account for rendered message lines (e.g., set `numRendered = 1`) to prevent layout jumping and preserve exact pagination height.
