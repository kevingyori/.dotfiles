...
**Learning:** Empty states in paginated TUIs without contextual messaging leave users confused whether the list is completely empty or if their current search filter is hiding all items.
**Action:** Always provide explicit, context-aware empty state messages ("No items" vs "No search results") and ensure blank space fillers account for the rendered message to prevent layout jumping.
