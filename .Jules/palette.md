...
## 2026-03-23 - [Differentiating Empty States]
**Learning:** In TUI applications, presenting a completely blank list or generic empty message can confuse users, especially when they are filtering content. Differentiating between "No items match your search" and "No items managed yet" provides immediate clarity on whether the overall dataset is empty or if it's just the current filter yielding no results.
**Action:** Always provide contextually specific empty state messages for filtered versus unfiltered list views, and ensure layout padding accounts for the rendered message to prevent UI jumping.
