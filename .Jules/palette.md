...
## 2026-05-18 - [Contextual Empty States]
**Learning:** In TUI applications, empty state messaging should be contextually differentiated to clarify whether the overall list is completely empty (e.g., 'No items managed yet') versus when a search filter simply yields no results.
**Action:** Replace missing empty states with helpful guidance and call-to-actions (e.g., 'No domains managed yet. Press 'a' to add one.') while adjusting blank space filler loops (numRendered = 1) to prevent layout jumping and preserve exact pagination height.
