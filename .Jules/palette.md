...
## 2024-05-18 - [Differentiating Empty States in TUI]
**Learning:** In TUI applications, empty state messaging should be contextually differentiated to clarify whether the overall list is completely empty (e.g., 'No items managed yet') versus when a search filter simply yields no results (e.g., 'No items match your search'). This helps users understand the state of their data and what actions to take.
**Action:** Always provide context-aware empty state messages in lists with search/filter capabilities.
