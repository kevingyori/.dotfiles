...
func (m Model) renderMainView() string {
	...
	if len(paginatedDomains) == 0 {
		if m.ui.searchInput.Value() != "" {
			b.WriteString(helpStyle.Render("No items match your search.") + "\n")
		} else {
			b.WriteString(helpStyle.Render("No items managed yet.") + "\n")
		}
	} else {
	...
	if numRendered == 0 {
		numRendered = 1 // Account for empty state message
	}
