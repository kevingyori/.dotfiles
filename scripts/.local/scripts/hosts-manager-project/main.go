...
func (m Model) renderMainView() string {
	...
	if len(paginatedDomains) == 0 {
		...
		if m.ui.searchInput.Value() != "" {
			emptyMsg = helpStyle.Render("No domains match your search.")
		} else {
			emptyMsg = helpStyle.Render("No domains managed yet. Press 'a' to add one.")
		}
		b.WriteString(emptyMsg + "\n")

		// Fill remaining space accounting for the empty message
		numRendered := 1
