...
// Useful for asynchronous operations that need safe deep copies to prevent data races.
...
// Performance optimization: Use for read-only operations (like UI loops) to prevent O(N) memory allocations per tick.
