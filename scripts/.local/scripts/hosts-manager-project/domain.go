...
// Items returns a direct read-only reference to the domains slice.
// Ideal for UI loops to prevent O(N) memory allocations per render tick.
...
func (dl *DomainList) Filter(query string) []Domain {
	if query == "" {
		return dl.Items() // Use Items() for zero-allocation when unfiltered
	}
